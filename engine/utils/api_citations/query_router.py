...
- Industry queries → web search first
...
APIName = Literal['crossref', 'semantic_scholar', 'web_search']
...
class QueryRouter:
    ...
    def get_api_chain(self, query_type: QueryType) -> List[APIName]:
        ...
        if query_type == 'industry':
            ...
            return ['web_search', 'semantic_scholar', 'crossref']

        elif query_type == 'academic':
            # Prioritize academic sources for scholarly queries
            return ['crossref', 'semantic_scholar', 'web_search']

        else:  # mixed
            # Balanced approach for mixed queries
            return ['semantic_scholar', 'web_search', 'crossref']
